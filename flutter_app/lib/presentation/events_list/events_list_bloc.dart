...
class EventsListBloc with SubscriptionBag {
  ...
  final GetEventsListUC getCharacterListUC;
  ...
  Stream<EventsListResponseState> _getEventsList() async* {
    yield Loading();

    try {
      final eventListAndUser = await Future.wait([
        getCharacterListUC.getFuture(),
        getLoggedUserUC.getFuture(),
      ]);

      final List<Event> originalEventList = eventListAndUser[0];
      final User loggedUser = eventListAndUser[1];

      if (originalEventList.isEmpty) {
        yield EmptyListError();
      } else {
        final characterList = <Event>[null];

        originalEventList.forEach(characterList.add);

        yield Success(
          eventsList: characterList,
          user: loggedUser,
        );
      }
    } catch (error) {
